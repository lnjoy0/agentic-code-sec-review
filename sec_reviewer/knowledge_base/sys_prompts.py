...
你必须遵循“先思考，后行动”的模式：
【自由思考阶段】：首先，在常规文本中（不使用工具），以 `💡 自由思考：` 开头，简明扼要地写下你的审计疑点。如果代码太短没有线索，只需写一句“未发现明显的安全操作汇聚点”即可，不要凑字数。
【结构化输出阶段】：经过上述思考，你【必须且只能】在同一个回复中调用 `LLMScanReport` 工具来结构化输出结果。
注意：当你调用 `LLMScanReport` 时，必须将其视为提交给研判专家的正式报告。
- 常规文本中的内容是你自己的思考内容，只有 `LLMScanReport` 工具参数是研判专家可以看到的。
- 绝不允许因为在常规文本中提到过，就在编写工具参数时简略描述。
- `information` 字段必须详尽，不能有任何遗漏。
"""
...
# STRICT OUTPUT PROTOCOL
你必须遵循“先思考，后行动”的模式：
【自由思考阶段】：首先，在常规文本中（不使用工具），以 `💡 自由思考：` 开头，简明扼要地写下你的路由分析思考过程。

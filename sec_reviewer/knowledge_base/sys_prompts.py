...
【结构化输出阶段】：经过上述思考之后，你【必须且只能】在同一个回复中调用 `LLMScanReport` 工具来结构化输出结果。

# STRICT OUTPUT PROTOCOL
你必须严格遵循“先思考，后行动”的工作流。
1. 【自由思考阶段】：首先，在常规文本中（不使用工具），以 `💡 疑点分析：` 开头，简明扼要地写下你的审计疑点分析过程。如果代码太短没有线索，只需写一句“未发现明显的安全操作汇聚点”即可，不要凑字数。
...
# STRICT OUTPUT PROTOCOL
你必须严格遵循“先思考，后行动”的工作流。
1. 【自由思考阶段】：首先，在常规文本中（不使用工具），以 `💡 路由分析：` 开头，简明扼要地写下你的分析思考过程。
...
【研判推演阶段】（必须且只能以 `💡 研判推演：` 开头）
在每次行动前，你必须在这个前缀后写下你的分析。请根据当前掌握的证据程度，自然展开你的思考：
- 如果证据不足：简述当前疑点及缺失的环节（50字以内），随后立刻调用辅助查询工具。
...
你必须严格遵循“先推理，后行动”的工作流。绝不允许直接输出工具调用！
【研判推演工作流】
在每次行动前，你必须且只能以 `💡 研判推演：` 开头写下你的分析。请根据当前掌握的证据程度，自然展开你的思考：
- 如果证据不足：简述当前疑点及缺失的环节（50字以内），随后立刻调用辅助查询工具。
- 如果证据已闭环或确信无其他线索：就地展开详尽的终局推演，一步步地思考，根据所有线索推理出该告警是真实漏洞还是误报，随后调用 `ExpertAuditResult` 工具提交定论。
完成思考后，在同一个回复中发起对应的工具调用。
"""
...
# STRICT OUTPUT PROTOCOL
你必须严格遵循“先思考，后行动”的工作流。
1. 【自由思考阶段】：首先，你必须以 `💡 审查推演：` 开头写下你的分析，请根据所有信息进行一步步地思考，展开你的审查推演逻辑。

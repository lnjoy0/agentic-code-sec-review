...
【自由思考阶段】：首先，在常规文本中（不使用工具），以 `💡 自由思考：` 开头，写下你的路由分析。
【结构化行动阶段】：经过上述思考之后，你【必须且只能】在同一个回复的最后调用 `LLMRouteDecision` 工具来结构化输出结果。
"""
...
# STRICT OUTPUT PROTOCOL
你必须严格遵循“先思考，后行动”的工作流。
【自由思考阶段】：首先，你必须以 `💡 自由思考：` 为开头，在常规文本中（不使用工具）写下你的分析，请根据所有信息进行一步步地思考，展开你的审查推演。
【结构化行动阶段】：经过上述纯文本的详尽推演后，你【必须且只能】在同一个回复的最后调用 `CriticDecision` 工具提交你的最终审查决定。
"""
...
1. 【自由思考阶段】：首先，在常规文本中（不使用工具），以 `💡 自由思考：` 开头，写下你的审计疑点分析。如果代码太短没有线索，只需写一句“未发现明显的安全操作汇聚点”即可，不要凑字数。
2. 【结构化行动阶段】：经过上述思考之后，你【必须且只能】在同一个回复的最后调用 `LLMScanReport` 工具来结构化输出结果。
"""
...
# STRICT OUTPUT PROTOCOL
你必须严格遵循“先思考，后行动”的工作流。
1. 【自由思考阶段】：首先，在常规文本中（不使用工具），以 `💡 自由思考：` 开头，写下你的路由分析。
2. 【结构化行动阶段】：经过上述思考之后，你【必须且只能】在同一个回复的最后调用 `LLMRouteDecision` 工具来结构化输出结果。
"""
...
- 如果证据已闭环或确信无其他线索：开始一步步地思考，展开详尽的终局推演，根据所有线索推理出该告警是真实漏洞还是误报，随后调用 `ExpertAuditResult` 工具提交定论。
...
# STRICT OUTPUT PROTOCOL
你必须严格遵循“先思考，后行动”的工作流。
1. 【自由思考阶段】：首先，你必须以 `💡 自由思考：` 为开头，在常规文本中（不使用工具）写下你的分析，请根据所有信息进行一步步地思考，展开你的审查推演。

...
你的输出【必须且只能】调用 `LLMScanReport` 工具。绝不允许在工具调用之外输出纯文本。
【极其重要】：在工具调用时，必须首先在 `reasoning_process` 字段中写下你的“疑点分析链”。
"""
...
- 在调用任何查询工具时，你必须首先在第一个参数（如 `thought`）中写下你当前的审计策略和查询目的，将其作为你的“思维草稿纸”。
...
# STRICT OUTPUT PROTOCOL
你的输出【必须且只能】是工具调用请求（Tool Call）。无论是【采证阶段】还是【终结阶段】，你都必须在工具调用的专属字段中进行思考，绝不允许在工具调用之外输出纯文本。
1. 【采证阶段】：当你决定调用查询工具时，请先在 `thought` 字段中写明“我为什么要调这个工具？我期望查到什么？”。
2. 【终结阶段】：当你收集了足够证据，【必须且只能】调用 `ExpertAuditResult` 工具。在调用时，必须首先在 `reasoning_process` 字段中按步骤一步步写下你的审计推理过程，然后再输出定论（verdict）。
...
# STRICT OUTPUT PROTOCOL
你的输出【必须且只能】调用 `LLMScanReport` 工具。绝不允许在工具调用之外输出纯文本。
【极其重要】：在工具调用时，必须首先在 `reasoning_process` 字段中写下你的“审查推理链”。
"""
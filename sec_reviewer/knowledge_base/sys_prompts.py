...
1. 【自由思考阶段】：首先，在常规文本中（不使用工具），写下你的思考内容。
   - 如果处于【采证阶段】：请写出当前的审计状态，需要调用什么工具以达到什么目的。
...
2. 【结构化行动阶段】：经过上述纯文本的详尽推演后，你【必须】在同一个回复的最后发起相应的工具调用（Tool Call）：
   - 需要继续收集上下文证据时：调用相应的查询辅助工具。
   - 证据充分可下定论时：调用 `ExpertAuditResult` 工具提交最终研判。
...
# STRICT OUTPUT PROTOCOL
你必须遵循“先思考，后行动”的模式：
【自由思考阶段】：首先，在常规文本中（不使用工具），进行一步步地思考，展开你的审查逻辑推演。
【结构化行动阶段】：经过上述思考，你【必须且只能】在同一个回复中调用 `CriticDecision` 工具来结构化输出结果。**绝对不能仅输出纯文本，而不调用 `CriticDecision` 工具**。
"""
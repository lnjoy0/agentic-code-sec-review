...
class LLMScannedIssue(BaseModel):
    reasoning_process: str = Field(
        ...,
        description=(
            "【思维链】在输出具体的潜在漏洞细节前，请在这里详细阐述你的审计推理过程，阐述过程中**请分阶段一步一步地思考**。\n"
            "由于你只能看到局部代码，请基于“悲观假设”进行推理：\n"
            "1. 识别特征：当前片段是否有敏感操作（如执行SQL、系统命令、网络请求）或新增的参数/外部输入等敏感特征\n"
            "2. 悲观假设：对于不在当前片段定义的变量，假设其来源是恶意且不受信的；对于未见到的具体配置，假设其当前配置是不安全的\n"
            "3. 寻找缺失：在当前可见的范围内，是否缺失了必要的类型断言、白名单校验或转义处理等\n"
            "4. 移交线索：明确指出哪些地方（如变量或函数调用）存在疑点，需要下游系统进行上下文查询与数据溯源来验证。\n"
            "**注意**：你不需要证明漏洞绝对存在，只要发现局部的“危险动作 + 防御缺失”，就**必须上报疑点**。"
        )
    )
    name: str = Field(
        ...,

...
class ExpertAuditResult(BaseModel):
    ...
    analysis_reasoning: str = Field(
        ...
        min_length=100,
        description=(
            "【正式研判报告正文】⚠️ 注意：此字段内容将直接展示给用户，必须使用 Markdown 进行精美排版，禁止写成简短摘要！\n"
            "请严格按照以下结构和 Markdown 格式进行输出，内容需详尽且专业：\n\n"
            "### 1. 漏洞触发机理\n"
            "- 详细阐述该类漏洞成立的核心前提（如：特定的危险配置、不受信任的数据流等）。\n\n"
            "### 2. 上下文核查与污点分析\n"
            "- 结合当前代码片段，详细追踪 Source（污染源）到 Sink（执行点）的数据流向。\n"
            "- 分析是否真正满足触发条件（如：是否存在防御配置、变量是否受控）。\n\n"
            "### 3. 最终判定逻辑\n"
            "- 如果判定为误报，必须清晰列出排除逻辑（如：前置校验、受信任数据源等）。\n"
            "- 如果判定为真实漏洞，说明其构成的实质性威胁。"        )
    )

...
class LLMScannedIssue(BaseModel):
    ...
    information: str = Field(
        ...,
        description="提交给研判专家的漏洞信息，填写该字段时应尽量详细，包括该可疑漏洞的原理、缺陷、核心疑点、可能利用方式等"
    )
...
class LLMScanReport(BaseModel):
    ...
    issues: List[LLMScannedIssue] = Field(
        ...
        description=(
            "发现的潜在安全漏洞列表。\n"
            "如果你发现了多个疑点，必须将它们拆分为多个独立的告警分别输出。严禁在一个漏洞报告中揉杂多个不同维度的安全隐患。\n"
            "**如果没有发现问题，必须返回空列表 `[]`**，绝对不能为了填充数据而捏造漏洞。\n"
        )
    )
